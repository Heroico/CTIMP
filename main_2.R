...
calc_stats <- function(res_, names_, corr_fold, sample_size_fold) {
	f_ <- apply(corr_fold, 1, sum)
	#non_zero_folds <- nnzero(f_)
	folds <- dim(corr_fold)[1]
	zscore_fold <- matrix(0, nrow(corr_fold), ncol(corr_fold))

	r <- list()
	for (i in 1:length(res_)) {

		zscore_fold[,i] <- atanh(corr_fold[,i])*sqrt(sample_size_fold[,i] - 3)

		r[[i]] <- data.frame(stringsAsFactors = FALSE,
				R2 = calc_R2(res_[[i]][,1], res_[[i]][,2]),
				n = dim(res_[[i]])[1],
				tissue = names_[i])
	}

	zscore_est <- apply(zscore_fold, 2, sum) / sqrt(folds)
	zscore_pval <- 2*pnorm(abs(zscore_est), lower.tail = FALSE)
	rho_avg <- apply(corr_fold, 2, sum) / folds

	do.call(rbind, r) %>%
		mutate(zscore_pval = zscore_pval, rho_avg_squared = rho_avg**2, rho_avg=rho_avg)
}

calc_corr_fold <- function(ans, X_all, Y_all) {
	fold_res = multi_mse(ans$est, X_all, Y_all)
	corr_ <- rep(0, length(fold_res))
	for (i in 1:length(fold_res)) {
		corr_[i] <- ifelse(sd(fold_res[[i]][,2]) != 0, cor(fold_res[[i]][,2], fold_res[[i]][,1]), 0)
	}
	corr_
}
...
#FOLDER <- "/home/numa/Documents/Projects/3rd/CTIMP"
# FOLDER <- "/scratch/abarbeira3/v8_process/ctimp/ctimp_scratch/ENSG00000186092.4"
# dose_path = file.path(FOLDER,"x.txt")
# info_path = file.path(FOLDER, "info.txt")
# Yt_path= file.path(FOLDER, "y")
# Yt = dir(Yt_path)
# ntune = 50
# gene_id = "test"
# outdir = file.path(FOLDER,"out")
# fold = 5
# seed = 0
parser <- ArgumentParser(description="train CTIMP")
parser$add_argument("-dosage")
parser$add_argument("-info")
parser$add_argument("-expression")
parser$add_argument("-ntune")
...
dose_path = args$dosage
info_path = args$info
Yt_path = args$expression
Yt = dir(args$expression)
ntune = as.numeric(args$ntune)
gene_id = args$gene_id
outdir = args$outdir
fold = as.integer(args$nfold)
...
#	for(j in 3:ncol(dose)){
	...
	corr_fold = matrix(0, fold, T_num)
	sample_size_fold = matrix(0, fold, T_num)
	for(f in 1:fold){
		cat("Fold ", f, "\n")
		bgt = Sys.time()
		...
		for(t in 1:T_num){
			...
			X_train[[t]] = apply(as.matrix(dose[X_train_tmp,-1]),2,as.numeric)
			Y_train[[t]] = Y[[t]][Y_train_tmp, 2]
			X_tune[[t]] = apply(as.matrix(dose[X_tuning_tmp,-1]),2,as.numeric)
			Y_tune[[t]] = Y[[t]][Y_tuning_tmp, 2]
			X_test[[t]] = apply(as.matrix(dose[X_test_tmp, -1]),2,as.numeric)
		...
		corr_fold[f,] = calc_corr_fold(ans, X_test, Y_test)
		sample_size_fold[f, ] = unlist(lapply(Y_test, length))
	...
	cat("saving gene data\n")
	save(single_res_test, single_lam, single_theta_est, multi_res_test, multi_lam, multi_theta_est, res_tune, rec_lamv, file = paste0(outdir,'/', gene_id, ".RData"))
	...
	for(t in 1:T_num){
		...
		X_all[[t]] = apply(as.matrix(dose[X_all_tmp,-1]),2,as.numeric)
		Y_all[[t]] = Y[[t]][,2]
	}
	# initial values 
	single_initial_est = matrix(0, ncol(X_train[[1]]), T_num)
	...
	cat("saving\n")
	names_ <- gsub(".txt", "", Yt)
	write.table(downstream_est, paste0(outdir, "/", gene_id, ".m.est"), quote=F, row.names=F, col.names=c("SNP", "REF.0.", "ALT.1.", names_))
	downstream_stats <- calc_stats(multi_all_res, names_, corr_fold, sample_size_fold)
	write.table(downstream_stats, paste0(outdir, "/", gene_id, ".m.stats"), quote=F, row.names=F)

#####
	# single_p_est = data.frame(info[,1:3], single_initial_est)
	# write.table(single_p_est, paste0(outdir, "/", gene_id, ".s.est"), quote=F, row.names=F, col.names=c("SNP", "REF.0.", "ALT.1.", names_))
#####
	...
	save(multi_all_res, single_all_res, final.lam, ans, file=paste0(outdir, "/", gene_id, "_prediction_on_all_data.RData"))

	# x <- ans$est
	# colnames(x) <- paste0("Y",1:22)
}
